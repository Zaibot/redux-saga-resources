...
export function httpMiddleware(api: IHttpApiHandler, path: string) {
  return (descriptor) => {
    ...
    const fnReset = sagaReset(api, path, descriptor);
    ...
    const mapping = {
      ...
      [descriptor.actions.RESET]: fnReset
    };
...
function sagaCreate(api: IHttpApiHandler, path: string, descriptor: IResource) {
...
function sagaRead(api: IHttpApiHandler, path: string, descriptor: IResource) {
  return next => function*(action) {
...
function sagaUpdate(api: IHttpApiHandler, path: string, descriptor: IResource) {
  return next => function*(action) {
    const key = descriptor.data.id(action.payload.item);
    const item = stripFields(action.payload.item);
    const { res, cancel } = yield race({
      res: api.put(`${path}/${key}`, { ...item,
        id: key
      }),
...
function sagaDelete(api: IHttpApiHandler, path: string, descriptor: IResource) {
  return next => function*(action) {
...
function sagaList(api: IHttpApiHandler, path: string, descriptor: IResource) {
  return next => function*(action) {
    const {
      res,
      cancel
...
function sagaReset(api: IHttpApiHandler, path: string, descriptor: IResource) {
  return next => function*(action) {
    yield* next();
  };
}